...
export default function HomeScreen() {
  ...
  const scanDocument = async () => {
    console.log('HomeScreen (iOS): User tapped Scan Document button');
    ...
    try {
      ...
      console.log('HomeScreen (iOS): Camera result:', result);
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        console.log('HomeScreen (iOS): Adding new document:', newDocument.name);
        setDocuments([newDocument, ...documents]);
      } else {
        console.log('HomeScreen (iOS): Camera scan cancelled');
      }
    } catch (error) {
      console.error('HomeScreen (iOS): Error scanning document:', error);
      Alert.alert('Error', 'Failed to scan document. Please try again.');
    }
  };

  const importFromGallery = async () => {
    console.log('HomeScreen (iOS): User tapped Import from Gallery button');
    ...
    try {
      ...
      console.log('HomeScreen (iOS): Gallery result:', result);
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        console.log('HomeScreen (iOS): Adding imported document:', newDocument.name);
        setDocuments([newDocument, ...documents]);
      } else {
        console.log('HomeScreen (iOS): Gallery import cancelled');
      }
    } catch (error) {
      console.error('HomeScreen (iOS): Error importing from gallery:', error);
      Alert.alert('Error', 'Failed to import document. Please try again.');
    }
  };

  const viewDocument = (doc: ScannedDocument) => {
    console.log('HomeScreen (iOS): User tapped to view document:', doc.name);
  ...
  const confirmDeleteDocument = (docId: string) => {
    console.log('HomeScreen (iOS): User requested to delete document:', docId);
  ...
  const deleteDocument = () => {
    if (documentToDelete) {
      console.log('HomeScreen (iOS): Deleting document:', documentToDelete);
